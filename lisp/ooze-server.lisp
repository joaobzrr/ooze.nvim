           ...
           (handler-case
             ...
             (error (c)
               (when *listening-socket*
                 (format *error-output*
                         "Error in server accept loop: ~a~%" c))
...
(defun stop-server ()
  ...
  (cond
    ((and *server-thread* (bt:thread-alive-p *server-thread*))
